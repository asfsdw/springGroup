	    	...
	    	$.ajax({
	    		...
	    		success:function(res) {
	    			if(res.indexOf("/") != -1) {
	    				...
	    				str += '</tr>';
	    				str += '<th>성별</th>';
	    				<c:set var="imsi" value="${fn:indexOf(res,'남자')}" />
	    				<c:if imsi != -1><c:set var="imsi" value="남자"/></c:if>
	    				<c:if imsi == -1><c:set var="imsi" value="여자"/></c:if>
	    				str += '<td><input type="radio" name="gender" value="남자" <c:if test="${imsi == \'남자\'}">checked</c:if> />남자 ';
	    				str += '<input type="radio" name="gender" value="여자" <c:if test="${imsi == \'여자\'}">checked</c:if> />여자</td>';
	    				str += '</tr>';
	    				str += '<th>주소</th>';
...
<body>
	...
	<div class="container">
		...
		<div class="row text-center">
			...
			<div class="col"><a href="javascript:loginList2()" class="btn btn-success btn-sm">회원리스트 동기식</a></div>
			<div class="col"><a href="javascript:loginList3()" class="btn btn-success btn-sm">회원리스트 vector</a></div>
