	      ...
	      ${st.count} : 
		    ...
		    <c:if test="${extName == 'txt'}">텍스트문서파일</c:if>
		  	<c:if test="${extName == 'jpg' || extName == 'gif' || extName == 'png'}">
		  	  <img src="${ctp}/images/pds/${file}" width="150px">
		  	</c:if>
		  	: <a href="${ctp}/images/pds/${file}" download="${file}" class="text-decoration-none" title="다운로드">${file}</a>
		  	: <a href="JavaFileDownload.study?fName=${file}" class="btn btn-info btn-sm">다운로드(java)</a>
